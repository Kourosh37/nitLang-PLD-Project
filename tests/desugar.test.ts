...
test("logical sugar becomes lazy conditional expression", () => {
  expect(lower("a and b or c").body).toMatchObject([{ expression: { kind: "ConditionalExpression",
    condition: { kind: "ConditionalExpression", condition: { name: "a" }, consequent: { name: "b" }, alternative: { value: false } },
    consequent: { value: true }, alternative: { name: "c" } } }]);
});
test("range has once-only boundaries and hygienic names with a separate body scope", () => {
  ...
  expect(core.body[0]).toMatchObject({ kind: "BlockStatement", body: [
    { kind: "LetDeclaration", name: { name: "@rangeStart0" }, initializer: { kind: "CallExpression" } },
    { kind: "LetDeclaration", name: { name: "@rangeEnd1" } },
    { name: { name: "i" } }, { kind: "WhileStatement", body: { body: [{ kind: "BlockStatement" }, { kind: "AssignmentStatement", target: { name: "i" } }] } },
  ] });
...
test("nested function and class sugars are fully removed", () => {
  ...
  for (const removed of ["ForStatement", "GroupExpression", "EmptyStatement", '"operator":"and"']) expect(json).not.toContain(removed);
});
