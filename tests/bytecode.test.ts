...
function code(source: string) {
  const result = check(new SourceFile("vm.nit", source));
...
test("compiler emits readable stack code and patched branch targets", () => {
  ...
  for (const instruction of chunk.instructions)
    if (instruction.op === "JUMP" || instruction.op === "JUMP_IF_FALSE") {
      expect(instruction.target).toBeGreaterThanOrEqual(0);
      expect(instruction.target).toBeLessThanOrEqual(chunk.instructions.length);
    }
  expect(disassemble(chunk)).toContain("JUMP_IF_FALSE");
  expect(disassemble(chunk)).toContain("BINARY +");
});
test("bytecode rejects unsupported checked constructs before VM execution", () => {
  for (const source of ["func f()={} f()", "let x=[1]", "class C {} new C()"])
    expect(() => code(source)).toThrow("bytecode backend does not support");
});
