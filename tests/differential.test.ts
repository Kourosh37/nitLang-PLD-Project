...
function outputs(text: string) {
  ...
  const a = run(source, (s) => tree.push(s)),
    b = runVm(source, (s) => vm.push(s));
  if (!a.ok || !b.ok)
    throw new Error(
      `Unexpected differential rejection: ${!a.ok ? a.diagnostic.message : !b.ok ? b.diagnostic.message : ""}`,
    );
  return { tree, vm };
}
for (const [name, source] of [
  ...
  test(`interpreter and VM agree: ${name}`, () => {
    const result = outputs(source);
    expect(result.vm).toEqual(result.tree);
  });
test("example loop is identical on both backends", async () => {
  const text = await Bun.file(new URL("../examples/02-control-flow.nit", import.meta.url)).text();
  expect(outputs(text)).toEqual({ tree: ["15", "true"], vm: ["15", "true"] });
});
