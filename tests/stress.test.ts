...
test("showcase combines representative tree-interpreter features", async () => {
  const source = await Bun.file(new URL("../examples/09-showcase.nit", import.meta.url)).text();
  expect(output(source)).toEqual(["[0, 1, 1, 2, 3, 5, 8]", "NITLang", "ready"]);
});

test("changed names values and nesting preserve semantics", () => {
  const source =
    "let alpha=3 func maker(delta:int)={return lambda (item:int)->alpha+delta+item} let operation=maker(4) { let alpha=999 print(operation(5)) } class Root { let data:int func init(v:int)={this.data=v} func read():int={return this.data} } class Leaf extends Root {} let instance:Root=new Leaf(8) print(instance.read())";
  expect(output(source)).toEqual(["12", "8"]);
});

test("exceptions from inherited methods inside loops reach lexical catches", () => {
  const source =
    'class A { func act(x:int):int={if x==1 then {throw "hit"} return x} } class B extends A {} let value:A=new B() for index in range(0,3) { try { print(value.act(index)) } catch issue { print(issue) } }';
...
test("large reasonable recursion and loops complete", () => {
  expect(
    output(
      "func sum(n:int):int={if n==0 then {return 0}else{return n+sum(n-1)}} print(sum(100)) let i=0 while i<1000 do {i=i+1} print(i)",
    ),
  ).toEqual(["5050", "1000"]);
});
...
test("VM stress subset agrees with tree execution", () => {
  const source =
    "let total=0 for number in range(0,20) { if number>=10 and number!=15 then {total=total+number} } print(total)";
  const tree: string[] = [],
    vm: string[] = [];
...
test("check accepts the complete example without executing it", async () => {
  const source = await Bun.file(new URL("../examples/09-showcase.nit", import.meta.url)).text();
  expect(check(new SourceFile("09-showcase.nit", source)).ok).toBe(true);
});
