...
describe("token vocabulary", () => {
  test("all specified keywords and complete identifiers", () => {
    const words =
      "let func return lambda if then else while do for in range and or not ref class extends this new try catch throw match int bool string void true false".split(
        " ",
      );
  ...
  test("every punctuation token", () => {
    const symbols = "+ - * / == != < > <= >= = := -> => ( ) { } [ ] , : ; .".split(" ");
    expect(tokens(symbols.join(" ")).map<string>((token) => token.kind)).toEqual([
      ...symbols,
      "eof",
    ]);
  });
  ...
  test("integer values, bounds, leading zeros and separate unary sign", () => {
    ...
    expect(result.filter((token) => token.kind === "integer").map((token) => token.value)).toEqual([
      0, 7, 9007199254740991, 42,
    ]);
    expect(result.map((token) => token.lexeme)).toEqual([
      "0",
      "007",
      "9007199254740991",
      "-",
...
describe("strings and trivia", () => {
  ...
  test("comments separate tokens and block comments do not nest", () => {
    expect(
      tokens("left/**/right / 2 // ignored\r\n+3 /* outer /* inner */ +4").map(
        (token) => token.lexeme,
      ),
    ).toEqual(["left", "right", "/", "2", "+", "3", "+", "4", ""]);
  });
  ...
  for (const text of ["", " \t\r\n", "// eof", "/* closed */", "\uFEFF", "\uFEFF // comment\n"]) {
    test(`trivia-only input ${JSON.stringify(text)}`, () => {
      ...
      expect(result[0]).toMatchObject({
        kind: "eof",
        lexeme: "",
        span: { start: { offset: text.length }, end: { offset: text.length } },
      });
    });
  }
...
describe("spans and state isolation", () => {
  test("BOM, comments, CRLF, CR and LF retain original positions", () => {
    ...
    expect(result.find((token) => token.lexeme === "print")?.span).toEqual({
      sourceName: "test.nit",
      start: { offset: 16, line: 2, column: 7 },
