...
describe("expressions and precedence", () => {
  ...
  for (const [lower, higher] of [
    ...
    test(`${higher} binds more tightly than ${lower} on either side`, () => {
      expect(expression(`a ${lower} b ${higher} c`)).toMatchObject({
        kind: "BinaryExpression",
        operator: lower,
        right: { kind: "BinaryExpression", operator: higher },
      });
      expect(expression(`a ${higher} b ${lower} c`)).toMatchObject({
        kind: "BinaryExpression",
        operator: lower,
        left: { kind: "BinaryExpression", operator: higher },
      });
    });
  }
  ...
  test("lambda body is greedy and may be nested", () => {
    expect(expression("lambda (x:int) -> lambda (y:int) -> x + y * 2")).toMatchObject({
      ...
      parameters: [{ name: { name: "x" } }],
      body: {
        kind: "LambdaExpression",
        parameters: [{ name: { name: "y" } }],
        body: { kind: "BinaryExpression", operator: "+", right: { operator: "*" } },
      },
    });
    expect(expression("(lambda (x:int) -> x + 1)(2)")).toMatchObject({
      kind: "CallExpression",
      callee: { kind: "GroupExpression" },
    });
  ...
  test("lists, construction and references preserve surface structure", () => {
    ...
    expect(expression("[1, [2], ref x]")).toMatchObject({
      ...
      elements: [
        { value: 1 },
        { kind: "ListExpression" },
        { kind: "ReferenceExpression", target: { name: "x" } },
      ],
    });
...
describe("declarations, statements and types", () => {
  ...
  test("functions, return annotations, nested declarations and bare return", () => {
    expect(
      program(
        "func outer(x:int):Fn(int)->int = { func inner(y:int):int = { return x + y } return inner }; func noop():void = { return; }",
      ).body,
    ).toMatchObject([
      {
        kind: "FunctionDeclaration",
        name: { name: "outer" },
        parameters: [{ name: { name: "x" }, annotation: { name: "int" } }],
        returnType: {
          kind: "FunctionType",
          parameters: [{ name: "int" }],
          result: { name: "int" },
        ...
        body: {
          body: [
            { kind: "FunctionDeclaration" },
            { kind: "ReturnStatement", value: { name: "inner" } },
          ],
        },
      },
      {
        ...
        body: { body: [{ kind: "ReturnStatement", value: null }] },
      },
    ]);
  });
  ...
  test("nested type syntax, right-associated function results and nominal types", () => {
    expect(
      program("let xs:List<Ref<Animal>> = []; let f:Fn(int,List<string>)->Fn()->bool = unknown")
        .body,
    ).toMatchObject([
      {
        annotation: {
          kind: "ListType",
          element: { kind: "ReferenceType", element: { kind: "NamedType", name: "Animal" } },
        },
      },
      {
        annotation: {
          kind: "FunctionType",
          parameters: [{ name: "int" }, { kind: "ListType", element: { name: "string" } }],
          result: { kind: "FunctionType", parameters: [], result: { name: "bool" } },
        },
      },
    ]);
  });
  ...
  test("ordinary assignment and write-through are distinct", () => {
    expect(program("x = 2; (object.field) = 3; getRef() := 4").body).toMatchObject([
      { kind: "AssignmentStatement", target: { kind: "Identifier", name: "x" } },
      {
        ...
        target: { kind: "MemberExpression", member: { name: "field" } },
      },
      {
  ...
  test("if, optional else, while and for sugar", () => {
    expect(
      program(
        "if a and b then {} else {}; if c then {} while x < 3 do { x = x + 1 } for i in range(start(), stop()) { print(i) }",
      ).body,
    ).toMatchObject([
      {
        kind: "IfStatement",
        condition: { kind: "BinaryExpression", operator: "and" },
        thenBranch: { body: [] },
        elseBranch: { body: [] },
      },
      ...
      { kind: "WhileStatement", body: { body: [{ kind: "AssignmentStatement" }] } },
      {
        kind: "ForStatement",
        variable: { name: "i" },
        start: { kind: "CallExpression", callee: { name: "start" } },
        end: { kind: "CallExpression", callee: { name: "stop" } },
      },
    ]);
  });
  ...
  test("try/catch and throw retain ordered statement bodies", () => {
    expect(
      program('try { throw "failure" } catch error { print(error); throw error }').body,
    ).toMatchObject([
      {
        kind: "TryStatement",
        body: { body: [{ kind: "ThrowStatement", value: { value: "failure" } }] },
        catchName: { name: "error" },
        catchBody: { body: [{ kind: "ExpressionStatement" }, { kind: "ThrowStatement" }] },
      },
    ]);
  });
  ...
  test("classes preserve inheritance, field and method declaration order", () => {
    expect(
      program(
        "class Point extends Base { let x:int func init(a:int) = { this.x = a } let y:int; func move():void = { this.x = this.x + 1 } }; class Empty {}",
      ).body,
    ).toMatchObject([
      {
        ...
        members: [
          { kind: "FieldDeclaration", name: { name: "x" }, annotation: { name: "int" } },
          {
            ...
            body: {
              body: [
                { kind: "AssignmentStatement", target: { object: { kind: "ThisExpression" } } },
              ],
            },
          },
  ...
  test("semicolons resolve greedy continuations across insignificant newlines", () => {
    ...
    expect(program("return\n1").body).toMatchObject([
      { kind: "ReturnStatement", value: { value: 1 } },
    ]);
  });
  ...
  test("semantic invalidity is deliberately not a parser concern", () => {
    expect(
      program(
        'let x:int = "wrong"; let x = missing; this; return 1; 1 := false; class C extends C {}',
      ).body,
    ).toHaveLength(6);
  });
});
...
describe("syntax errors and phase contracts", () => {
  ...
  test("match parses literal and wildcard patterns", () => {
    expect(expression("match true { true => 1; false => 0 }")).toMatchObject({
      kind: "MatchExpression",
      arms: [{ pattern: { kind: "BooleanPattern", value: true } }, { pattern: { value: false } }],
    });
    expect(expression('match 1 { -1 => "negative"; _ => "other" }')).toMatchObject({
      arms: [{ pattern: { value: -1 } }, { pattern: { kind: "WildcardPattern" } }],
    });
  });
  ...
  test("long left-associative chains parse without recursive left descent", () => {
    ...
    let operators = 0;
    while (node.kind === "BinaryExpression") {
      operators += 1;
      node = node.left;
    }
  ...
  test("AST spans include delimiters but exclude preceding trivia", () => {
    ...
    const slice = (span: { start: { offset: number }; end: { offset: number } }) =>
