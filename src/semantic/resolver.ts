...
export interface Resolution {
  readonly bindings: WeakMap<object, Binding>;
  readonly receivers: WeakMap<C.FunctionDeclaration, Binding>;
}
export function resolve(program: C.Program): Resolution {
  ...
  const defineBinding = (name: C.Identifier, mutable: boolean): Binding => {
    ...
    scope.define(binding, name.span);
    bindings.set(name, binding);
    return binding;
  };
  const use = (node: C.Node, name: string) => {
    bindings.set(node, scope.lookup(name, node.span));
  };
  const annotation = (type: C.TypeAnnotation): void => {
    if (type.kind === "NamedType") use(type, type.name);
      ...
      type.parameters.forEach(annotation);
      annotation(type.result);
    }
  };
  const parameters = (params: readonly C.Parameter[]) => {
    for (const p of params) {
      annotation(p.annotation);
      defineBinding(p.name, true);
    }
  };
  ...
  const expression = (node: C.Expression): void => {
    switch (node.kind) {
      ...
      case "ReferenceExpression":
        use(node.target, node.target.name);
        break;
      ...
      case "MemberExpression":
        ...
        break;
      case "NewExpression":
        use(node.className, node.className.name);
        node.arguments.forEach(expression);
        break;
      case "ListExpression":
      ...
      case "MatchExpression":
        expression(node.scrutinee);
        node.arms.forEach((arm) => expression(arm.expression));
        break;
    }
  };
  ...
  const statement = (node: C.Statement): void => {
    switch (node.kind) {
      case "BlockStatement":
        ...
        break;
      case "LetDeclaration":
        if (node.annotation !== null) annotation(node.annotation);
        expression(node.initializer);
        defineBinding(node.name, true);
        break;
      ...
      case "IfStatement":
        ...
        if (node.elseBranch !== null) block(node.elseBranch);
        break;
      case "WhileStatement":
        expression(node.condition);
        block(node.body);
      ...
      case "TryStatement":
        block(node.body);
        scoped(() => {
          defineBinding(node.catchName, true);
          node.catchBody.body.forEach(statement);
        });
        break;
    }
  };
