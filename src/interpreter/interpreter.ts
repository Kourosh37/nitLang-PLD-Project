...
import {
  ...
  formatPrimitive,
  requireBoolean,
} from "../runtime/primitive-operations";
import {
  intValue,
  boolValue,
  stringValue,
...
export class Interpreter {
  ...
  method(classValue: ClassValue, name: string): C.FunctionDeclaration | undefined {
    return (
      ...
      (classValue.parent === null ? undefined : this.method(classValue.parent, name))
    );
  }
  allFields(classValue: ClassValue): string[] {
    return [
      ...(classValue.parent === null ? [] : this.allFields(classValue.parent)),
      ...classValue.fields,
    ];
  }
  invoke(callee: RuntimeValue, args: readonly RuntimeValue[], span: SourceSpan): RuntimeValue {
    if (callee.kind === "bound-method") {
      ...
      const closure = {
        ...
        parameters: callee.method.parameters,
      ...
      closure.parameters.forEach((p, i) => {
        const value = args[i];
        if (value === undefined) throw new Error("Missing checked argument.");
        environment.define(this.id(p.name), this.store.allocate(value), p.span);
      });
      this.callDepth += 1;
      try {
        for (const statement of closure.body.body) this.statement(statement, environment);
        return VOID;
      } catch (error: unknown) {
  ...
  expression(node: C.Expression, environment: Environment): RuntimeValue {
    switch (node.kind) {
      ...
      case "Identifier":
        return this.store.read(environment.lookup(this.id(node), node.span), node.span);
      case "ThisExpression":
        return this.store.read(environment.lookup(this.id(node), node.span), node.span);
      case "ReferenceExpression":
        return referenceValue(environment.lookup(this.id(node.target), node.span));
      case "UnaryExpression":
        return applyUnary(node.operator, this.expression(node.operand, environment), node.span);
      ...
      case "ConditionalExpression":
        return this.expression(
          requireBoolean(this.expression(node.condition, environment), node.span)
            ? node.consequent
            : node.alternative,
          environment,
        );
      case "LambdaExpression":
        return { kind: "closure", parameters: node.parameters, body: node.body, environment };
      case "ListExpression":
        return listValue(node.elements.map((item) => this.expression(item, environment)));
      case "MemberExpression": {
        ...
        return {
          ...
          method,
          receiver: object,
          environment: object.classValue.environment,
        };
      }
      case "NewExpression": {
        const value = this.store.read(
          environment.lookup(this.id(node.className), node.span),
          node.span,
        );
        ...
        if (init !== undefined)
          this.invoke(
            ...
            node.arguments.map((a) => this.expression(a, environment)),
            node.span,
          );
      ...
      case "MatchExpression": {
        ...
        for (const arm of node.arms) {
          ...
          const matches =
            ...
            (pattern.kind === "IntegerPattern" &&
              value.kind === "int" &&
              value.value === pattern.value) ||
            (pattern.kind === "StringPattern" &&
              value.kind === "string" &&
              value.value === pattern.value) ||
            (pattern.kind === "BooleanPattern" &&
              value.kind === "bool" &&
              value.value === pattern.value);
      ...
      case "CallExpression": {
        if (node.callee.kind === "Identifier" && this.id(node.callee) === PRINT_ID) {
          ...
          this.output(formatPrimitive(this.expression(arg, environment), node.span));
          return VOID;
        }
        if (node.callee.kind === "Identifier" && this.id(node.callee) === -2) {
          ...
          if (callbackNode === undefined || listNode === undefined)
            throw new Error("Missing checked map arguments.");
          const callback = this.expression(callbackNode, environment),
            list = this.expression(listNode, environment);
          if (list.kind !== "list") throw new Error("Checked map list invariant failed.");
          return listValue(
            list.elements.map((element) => this.invoke(callback, [element], node.span)),
          );
        }
      ...
      default:
        throw new Error("Unsupported checked expression.");
    }
  }
  statement(node: C.Statement, environment: Environment): void {
    switch (node.kind) {
      case "BlockStatement": {
        ...
        node.body.forEach((s) => this.statement(s, child));
        break;
      }
      case "LetDeclaration": {
        const value = this.expression(node.initializer, environment);
      ...
      case "FunctionDeclaration": {
        ...
        this.store.write(
          location,
          { kind: "closure", parameters: node.parameters, body: node.body, environment },
          node.span,
        );
        break;
      }
      case "ReturnStatement":
        throw new ControlSignal(
          "return",
          node.value === null ? VOID : this.expression(node.value, environment),
          node.span,
        );
      case "ThrowStatement":
        throw new ControlSignal("throw", this.expression(node.value, environment), node.span);
      case "TryStatement": {
        try {
          this.statement(node.body, environment);
        } catch (error: unknown) {
          if (!(error instanceof ControlSignal) || error.kind !== "throw") throw error;
          const catchEnvironment = new Environment(environment);
          catchEnvironment.define(
            this.id(node.catchName),
            this.store.allocate(error.value),
            node.catchName.span,
          );
      ...
      case "AssignmentStatement": {
        if (node.target.kind === "MemberExpression") {
          ...
          if (object.kind !== "object")
            throw new Error("Checked field assignment invariant failed.");
          const location = object.fields.get(node.target.member.name);
          if (location === undefined) throw new Error("Missing checked field.");
          this.store.write(location, this.expression(node.value, environment), node.span);
          break;
        }
      ...
      case "ReferenceAssignmentStatement": {
        ...
        this.store.write(reference.target, this.expression(node.value, environment), node.span);
        break;
      }
      case "IfStatement":
        if (requireBoolean(this.expression(node.condition, environment), node.span))
          this.statement(node.thenBranch, environment);
        else if (node.elseBranch !== null) this.statement(node.elseBranch, environment);
        break;
      case "WhileStatement":
        while (requireBoolean(this.expression(node.condition, environment), node.span))
          this.statement(node.body, environment);
        break;
      default:
        throw new Error("Unsupported checked statement.");
    }
  }
  run(): void {
    ...
    for (const node of this.checked.program.body) {
      if (node.kind === "ClassDeclaration") {
        const methods = new Map(
          node.members
            .filter((m): m is C.FunctionDeclaration => m.kind === "FunctionDeclaration")
            .map((m) => [m.name.name, m]),
        );
        const fields = node.members
          .filter((m) => m.kind === "FieldDeclaration")
          .map((m) => m.name.name);
        let parent: ClassValue | null = null;
        if (node.parent !== null) {
          const candidate = this.store.read(
            environment.lookup(this.id(node.parent), node.span),
            node.span,
          );
          if (candidate.kind !== "class") throw new Error("Checked parent invariant failed.");
          parent = candidate;
        }
